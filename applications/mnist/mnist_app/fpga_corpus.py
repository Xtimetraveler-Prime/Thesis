...
def build_corpus_cases(frozen_root: str | Path) -> tuple[MnistFpgaCorpusCase, ...]:
    ...
    # Load each deployment once. The resulting static images are reused by all
    # 30 image schedules for that profile in the generated hardware include.
    ...
    for source_ordinal, entry in enumerate(entries):
        ...
        for profile_name in PROFILE_ORDER:
            ...
            name = (
                f"mnist08-{profile_name}-index{source_index:05d}-"
                f"{safe_reason}"
            )
...
) -> tuple[MnistFpgaStaticProfile, ...]:
    selected = tuple(cases)
    result: list[MnistFpgaStaticProfile] = []
    for profile_name in PROFILE_ORDER:
        match = next(case for case in selected if case.profile == profile_name)
        result.append(MnistFpgaStaticProfile.from_case(match))
    return tuple(result)
...
def write_corpus_systemverilog_include(
    ...
    """Write shared static profiles plus packed per-case external schedules."""
    ...
    lines = [
        ...
        "// Static deployment images are stored once per profile; cases select profile + schedule.",
    ...
    def emit(name: str, width: int, words: Sequence[int]) -> None:
        ...
        lines.append(
            f"localparam logic [{width - 1}:0] {name} [0:{len(values) - 1}] = '{{"
        )
    ...
    emit(
        "M12_3_CONFIG_WORDS",
        128,
        tuple(word for profile in profiles for word in _pad(profile.config_words, max_neurons)),
    )
    emit(
        ...
        tuple(
            ...
            for word in _pad(profile.initial_state_words, max_neurons)
        ),
    )
    emit(
        "M12_3_FORMAT_WORDS",
        16,
        tuple(word for profile in profiles for word in _pad(profile.format_words, max_formats)),
    )
    emit(
        "M12_3_SYNAPSE_WORDS",
        32,
        tuple(
            word for profile in profiles for word in _pad(profile.synapse_words, max_synapses)
        ),
    )
    emit(
        "M12_3_WEIGHT_ROWS",
        ...
        tuple(
            word
            for profile in profiles
            for word in _pad(profile.weight_rows, max_axons + 1)
        ),
    )
    emit(
        "M12_3_ROUTE_ROWS",
        ...
        tuple(
            word
            for profile in profiles
            for word in _pad(profile.route_rows, max_neurons + 1)
        ),
    )
    emit(
        "M12_3_ROUTE_TARGETS",
        ...
        tuple(
            word
            for profile in profiles
            for word in _pad(profile.route_targets, max_routes)
        ),
    )

    # Pack all external events back-to-back. Row pointers avoid the enormous
    # case*tick*max_events rectangular ROM that a naive 60-case expansion would
    # create while preserving exact order and multiplicity.
    external_counts: list[int] = []
    external_rows: list[int] = [0]
    ...
    for case in selected:
        ...
        for events in padded_ticks:
            ...
            external_words.extend(int(event) for event in events)
            external_rows.append(len(external_words))
    if not external_words:
        external_words.append(0)
        ...
        json.dumps(
            {
                ...
                "fpga_input_contract": (
                    "two-shared-static-profile-images-plus-packed-case-external-events; "
...
def validate_physical_corpus_suite(
    ...
    for record in records:
        ...
        golden = read_physical_fpga_trace_json(
            manifest_path.parent / str(record["golden_trace"])
        )
        physical = read_physical_fpga_trace_json(
            physical_dir / f"{case_id:02d}-{name}.physical.json"
        )
    ...
    for profile in PROFILE_ORDER:
        group = [report for report in reports if report["profile"] == profile]
        profile_summary[profile] = {
            "cases": len(group),
            "passed": sum(int(bool(report["passed"])) for report in group),
            "mismatches": sum(int(report["mismatch_count"]) for report in group),
        }
    for reason in ("both-correct", "profile-divergent", "both-wrong"):
        group = [report for report in reports if report["selection_reason"] == reason]
    ...
    suite = {
        ...
        "source_image_count": EXPECTED_SOURCE_CASES,
        "committed_tick_count": sum(16 for _ in reports),
