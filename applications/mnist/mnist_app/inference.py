...
from .config import OUTPUT_NEURONS, PRESENTATION_TICKS
...
def load_core_from_deployment(manifest_path: str | Path):
    ...
    if payload.get("schema") != "neuromorphic-twin-mnist-deployment-v1":
    ...
    synapses = read_weight_storage_json(storage_path).decode_synapses()
...
def infer_image(core, image: np.ndarray) -> InferenceResult:
...
def evaluate_dataset(core, images: np.ndarray, labels: np.ndarray) -> dict[str, object]:
