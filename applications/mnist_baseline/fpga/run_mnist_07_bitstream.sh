...
LOCAL_BUILD_DIR="$APP_DIR/build/mnist-07"
...
LOG_FILE="$LOCAL_BUILD_DIR/mnist_07_vivado.log"
JOBS="${MNIST_07_JOBS:-4}"

STAGE_ROOT="/tmp/neuromorphic_twin_rtl_${UID}/mnist_07"
...
if [[ -n "${HLS_PART:-}" && "$HLS_PART" != "$EXPECTED_PART" ]]; then
    echo "ERROR: MNIST-07 is frozen to $EXPECTED_PART, but HLS_PART=$HLS_PART" >&2
    exit 2
fi
if [[ ! "$JOBS" =~ ^[1-9][0-9]*$ ]]; then
    echo "ERROR: MNIST_07_JOBS must be a positive integer; got '$JOBS'." >&2
    exit 2
fi
...
SOURCE_FILES=(
    ...
    "$APP_DIR/scripts/generate_fpga_conformance.py"
)
for path in "${SOURCE_FILES[@]}"; do
    if [[ ! -f "$path" ]]; then
        echo "ERROR: missing MNIST-07 source file: $path" >&2
        exit 3
    fi
done
...
cp "$CORE_RTL/m12_3_multitick_capture_controller_v1.sv" "$CAPTURE_RTL"
...
python3 "$APP_DIR/scripts/generate_fpga_conformance.py" \
...
if grep -Eq 'M12_3_EXPECTED|RECURRENT_SCHEDULE|RECURRENT_EVENTS' "$CAPTURE_VECTORS"; then
    echo "ERROR: generated MNIST-07 FPGA include contains forbidden golden/recurrent schedule arrays." >&2
    exit 3
fi

printf 'MNIST-07 toolchain: Vivado %s\n' "$EXPECTED_VERSION"
printf 'MNIST-07 target part: %s\n' "$EXPECTED_PART"
printf 'MNIST-07 packaged HLS IP: %s\n' "$EXPECTED_VLNV"
printf 'MNIST-07 implementation jobs: %s\n' "$JOBS"
printf 'MNIST-07 cases: 2\n'
printf 'MNIST-07 committed ticks: 32\n'

echo
echo '=== MNIST-07 routed implementation + bitstream ==='
vivado -mode batch \
    -source "$VIVADO_TCL" \
    -tclargs \
    "$IP_REPO_DIR" \
...
for artifact in "$M12_BIT" "$M12_LTX" "$M12_XSA" "$M12_DCP"; do
    if [[ ! -s "$artifact" ]]; then
        echo "ERROR: required MNIST-07 implementation artifact missing: $artifact" >&2
        exit 4
    fi
done

cp "$M12_BIT" "$ARTIFACT_DIR/neuromorphic_twin_mnist_07.bit"
cp "$M12_LTX" "$ARTIFACT_DIR/neuromorphic_twin_mnist_07.ltx"
cp "$M12_XSA" "$ARTIFACT_DIR/neuromorphic_twin_mnist_07.xsa"
cp "$M12_DCP" "$ARTIFACT_DIR/neuromorphic_twin_mnist_07_routed.dcp"

if [[ ! -s "$GOLDEN_DIR/manifest.json" || ! -s "$GOLDEN_DIR/hardware_cases.tsv" ]]; then
    echo "ERROR: MNIST-07 golden case bundle is incomplete." >&2
    exit 4
fi
...
echo
echo 'MNIST-07 routed bitstream flow completed successfully.'
printf 'Bitstream: %s\n' "$ARTIFACT_DIR/neuromorphic_twin_mnist_07.bit"
printf 'Debug probes: %s\n' "$ARTIFACT_DIR/neuromorphic_twin_mnist_07.ltx"
