...
// M12.3 case-selectable multi-tick/recurrent physical capture controller.
//
...
// generated_m12_3_multitick_cases.svh; expected outputs remain host-side.
//
...
// does not change.
//
// Unlike the fixed reset-state M12.1 workload, M12.3 must test arbitrary
...
module m12_3_multitick_capture_controller_v1 (
    ...
    // Load/control mux. Every M12_3_* array here is an INPUT/load-image array;
    ...
    always_comb begin
        ...
        case (state)
            S_LOAD_CONFIG: begin
                config_we = 1'b1;
                config_wdata = M12_3_CONFIG_WORDS[
                    (active_case_id * M12_3_MAX_NEURONS) + load_index
                ];
            end
            S_LOAD_FORMAT: begin
                format_we = 1'b1;
                format_wdata = M12_3_FORMAT_WORDS[
                    (active_case_id * M12_3_MAX_FORMATS) + load_index
                ];
            end
            S_LOAD_SYNAPSE: begin
                synapse_we = 1'b1;
                synapse_wdata = M12_3_SYNAPSE_WORDS[
                    (active_case_id * M12_3_MAX_SYNAPSES) + load_index
                ];
            end
            S_LOAD_WEIGHT_ROW: begin
                row_we = 1'b1;
                row_wdata = M12_3_WEIGHT_ROWS[
                    (active_case_id * (M12_3_MAX_AXONS + 1)) + load_index
                ];
            end
            S_LOAD_ROUTE_ROW: begin
                route_row_we = 1'b1;
                route_row_wdata = M12_3_ROUTE_ROWS[
                    (active_case_id * (M12_3_MAX_NEURONS + 1)) + load_index
                ];
            end
            S_LOAD_ROUTE_TARGET: begin
                route_target_we = 1'b1;
                route_target_wdata = M12_3_ROUTE_TARGETS[
                    (active_case_id * M12_3_MAX_ROUTES) + load_index
                ];
            end
            S_RESET_PULSE: core_reset_start = 1'b1;
            S_LOAD_STATE: begin
                state_we = 1'b1;
                state_wdata = M12_3_INITIAL_STATE_WORDS[
                    (active_case_id * M12_3_MAX_NEURONS) + load_index
                ];
            end
            S_LOAD_EXTERNAL: begin
                external_we = 1'b1;
                external_wdata = M12_3_EXTERNAL_EVENTS[
                    (((active_case_id * M12_3_MAX_TICKS) + tick_index) *
                ...
                S_RESET_WAIT: begin
                    if (core_fault) begin
                        ...
                        if ((core_tick != 32'd0) || (recurrent_current_count != 13'd0)) begin
                            ...
                            // M12.3 initial state is intentionally loaded AFTER
