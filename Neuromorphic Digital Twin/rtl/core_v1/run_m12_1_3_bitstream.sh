...
LOCAL_BUILD_DIR="$SCRIPT_DIR/build/m12_1_3"
...
LOG_FILE="$LOCAL_BUILD_DIR/m12_1_3_vivado.log"
JOBS="${M12_1_3_JOBS:-4}"

STAGE_ROOT="/tmp/neuromorphic_twin_rtl_${UID}/m12_1_3"
...
CAPTURE_RTL="$STAGE_ROOT/m12_1_capture_controller_v1.sv"
CAPTURE_BD_RTL="$STAGE_ROOT/m12_1_capture_controller_bd_v1.v"
CAPTURE_VECTORS="$STAGE_ROOT/generated_m11_5_4_integrated_vectors.svh"
VIVADO_TCL="$STAGE_ROOT/create_m12_1_3_project.tcl"
...
if [[ -n "${HLS_PART:-}" && "$HLS_PART" != "$EXPECTED_PART" ]]; then
    echo "ERROR: M12.1.3 is frozen to $EXPECTED_PART, but HLS_PART=$HLS_PART" >&2
    exit 2
fi
if [[ ! "$JOBS" =~ ^[1-9][0-9]*$ ]]; then
    echo "ERROR: M12_1_3_JOBS must be a positive integer; got '$JOBS'." >&2
    exit 2
fi
...
SOURCE_FILES=(
    ...
    "$SCRIPT_DIR/m12_1_capture_controller_v1.sv"
    "$SCRIPT_DIR/m12_1_capture_controller_bd_v1.v"
    "$SCRIPT_DIR/vivado/create_m12_1_3_project.tcl"
...
for path in "${SOURCE_FILES[@]}"; do
    if [[ ! -f "$path" ]]; then
        echo "ERROR: missing M12.1.3 source file: $path" >&2
        exit 3
    fi
done
...
cp "$SCRIPT_DIR/m12_1_capture_controller_v1.sv" "$CAPTURE_RTL"
cp "$SCRIPT_DIR/m12_1_capture_controller_bd_v1.v" "$CAPTURE_BD_RTL"
cp "$SCRIPT_DIR/vivado/create_m12_1_3_project.tcl" "$VIVADO_TCL"

PYTHONPATH="$PROJECT_DIR/src${PYTHONPATH:+:$PYTHONPATH}" \
python3 "$PROJECT_DIR/examples/generate_m11_5_4_integrated_vectors.py" \
    --output "$CAPTURE_VECTORS"

printf 'M12.1.3 toolchain: Vivado %s\n' "$EXPECTED_VERSION"
printf 'M12.1.3 target part: %s\n' "$EXPECTED_PART"
printf 'M12.1.3 packaged HLS IP: %s\n' "$EXPECTED_VLNV"
printf 'M12.1.3 implementation jobs: %s\n' "$JOBS"
printf 'M12.1.3 no-space staging directory: %s\n' "$STAGE_ROOT"

echo
echo '=== M12.1.3 routed implementation + bitstream ==='
vivado -mode batch \
    -source "$VIVADO_TCL" \
    -tclargs \
    "$IP_REPO_DIR" \
...
for marker in \
    "M12.1.3 physical trace-capture block design validated successfully." \
    "M12.1.3 implementation completed successfully." \
    "M12.1.3 routed timing check passed:" \
    "M12.1.3 bitstream generated successfully."; do
    if ! grep -Fq "$marker" "$LOG_FILE"; then
        echo "ERROR: Vivado returned without expected M12.1.3 marker: $marker" >&2
        exit 4
    fi
done
...
XSA_FILE="$ARTIFACT_DIR/neuromorphic_twin_m12_1_3.xsa"
DCP_FILE="$ARTIFACT_DIR/neuromorphic_twin_m12_1_3_routed.dcp"
REQUIRED_REPORTS=(
...
for artifact in "$BIT_FILE" "$LTX_FILE" "$XSA_FILE" "$DCP_FILE"; do
    if [[ ! -s "$artifact" ]]; then
        echo "ERROR: required M12.1.3 artifact is missing or empty: $artifact" >&2
        exit 4
    fi
done
for report in "${REQUIRED_REPORTS[@]}"; do
    if [[ ! -s "$report" ]]; then
        echo "ERROR: required M12.1.3 report is missing or empty: $report" >&2
        exit 4
    fi
done

# Reuse the physical-capacity checker used for M11.6. M12 debug transport may
# add logic but must not exceed the frozen K26 implementation capacity.
python3 "$SCRIPT_DIR/check_m11_6_resources.py" \
    "$REPORT_DIR/utilization_impl.rpt" \
    "$REPORT_DIR/ram_utilization_impl.rpt"

echo
echo 'M12.1.3 routed bitstream flow completed successfully.'
printf 'Bitstream: %s\n' "$BIT_FILE"
