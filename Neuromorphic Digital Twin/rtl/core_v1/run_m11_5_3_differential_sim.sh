...
STAGE_ROOT="/tmp/neuromorphic_twin_rtl_${UID}/m11_5_3_differential"
SNAPSHOT="m11_5_3_phase_b_differential_sim"
VECTOR_FILE="$STAGE_ROOT/generated_m11_5_3_vectors.svh"
SIM_LOG="$STAGE_ROOT/xsim.log"
PASS_MARKER="M11.5.3 Python/RTL accumulator differential passed:"
...
cp "$SCRIPT_DIR/tb/tb_phase_b_synapse_accumulator_differential_v1.sv" "$STAGE_ROOT/tb/"

PYTHONPATH="$ROOT_DIR/src${PYTHONPATH:+:$PYTHONPATH}" \
    python3 "$ROOT_DIR/examples/generate_m11_5_3_vectors.py" \
    --output "$VECTOR_FILE"
...
xvlog --sv -i . \
    ...
    tb/tb_phase_b_synapse_accumulator_differential_v1.sv

xelab tb_phase_b_synapse_accumulator_differential_v1 \
    -snapshot "$SNAPSHOT" \
    -debug typical
...
if ! grep -Fq "$PASS_MARKER" "$SIM_LOG"; then
    echo "ERROR: XSIM returned without the M11.5.3 differential pass marker." >&2
    exit 1
fi

echo
echo "M11.5.3 Python-to-RTL differential simulation completed successfully."
