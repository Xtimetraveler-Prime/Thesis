...
LOCAL_BUILD_DIR="$SCRIPT_DIR/build/m12_5"
...
if [[ -n "${HLS_PART:-}" && "$HLS_PART" != "$EXPECTED_PART" ]]; then
    echo "ERROR: M12.5 is frozen to $EXPECTED_PART, but HLS_PART=$HLS_PART" >&2
    exit 2
fi
if [[ ! "$JOBS" =~ ^[1-9][0-9]*$ ]]; then
    echo "ERROR: M12_5_JOBS must be a positive integer; got '$JOBS'." >&2
    exit 2
fi
...
SOURCE_FILES=(
    ...
    "$SCRIPT_DIR/m12_5_characterization_capture_controller_v1.sv"
    "$SCRIPT_DIR/m12_5_characterization_capture_controller_bd_v1.v"
    "$SCRIPT_DIR/vivado/create_m12_5_project.tcl"
    "$SCRIPT_DIR/check_m11_6_resources.py"
    "$PROJECT_DIR/examples/generate_m12_4_broad_corpus.py"
)
for path in "${SOURCE_FILES[@]}"; do
    if [[ ! -f "$path" ]]; then
        echo "ERROR: missing M12.5 source file: $path" >&2
        exit 3
    fi
done
...
cp "$SCRIPT_DIR/m12_5_characterization_capture_controller_v1.sv" "$CAPTURE_RTL"
cp "$SCRIPT_DIR/m12_5_characterization_capture_controller_bd_v1.v" "$CAPTURE_BD_RTL"
cp "$SCRIPT_DIR/vivado/create_m12_5_project.tcl" "$VIVADO_TCL"
...
if grep -Eq 'M12_4_EXPECTED|RECURRENT_SCHEDULE|GOLDEN_STATE|GOLDEN_SPIKE' "$CAPTURE_VECTORS"; then
    echo "ERROR: generated M12.5 FPGA include contains forbidden golden/recurrent schedule arrays." >&2
    exit 3
fi

printf 'M12.5 toolchain: Vivado %s\n' "$EXPECTED_VERSION"
printf 'M12.5 target part: %s\n' "$EXPECTED_PART"
printf 'M12.5 packaged HLS IP: %s\n' "$EXPECTED_VLNV"
printf 'M12.5 implementation jobs: %s\n' "$JOBS"
printf 'M12.5 characterization cases: 22\n'
printf 'M12.5 characterization committed ticks: 166\n'
printf 'M12.5 no-space staging directory: %s\n' "$STAGE_ROOT"

echo
echo '=== M12.5 routed implementation + bitstream ==='
vivado -mode batch \
    -source "$VIVADO_TCL" \
    -tclargs \
    "$IP_REPO_DIR" \
...
for marker in \
    ...
    if ! grep -Fq "$marker" "$LOG_FILE"; then
        echo "ERROR: Vivado returned without expected M12.5 marker: $marker" >&2
        exit 4
    fi
done
...
XSA_FILE="$ARTIFACT_DIR/neuromorphic_twin_m12_5.xsa"
DCP_FILE="$ARTIFACT_DIR/neuromorphic_twin_m12_5_routed.dcp"
REQUIRED_REPORTS=(
...
for artifact in "$BIT_FILE" "$LTX_FILE" "$XSA_FILE" "$DCP_FILE"; do
    if [[ ! -s "$artifact" ]]; then
        echo "ERROR: required M12.5 artifact is missing or empty: $artifact" >&2
        exit 4
    fi
done
for report in "${REQUIRED_REPORTS[@]}"; do
    if [[ ! -s "$report" ]]; then
        echo "ERROR: required M12.5 report is missing or empty: $report" >&2
        exit 4
    fi
done
if [[ ! -s "$GOLDEN_DIR/manifest.json" ]]; then
    echo "ERROR: M12.5 golden corpus manifest is missing." >&2
    exit 4
fi
...
echo
echo 'M12.5 routed bitstream flow completed successfully.'
printf 'Bitstream: %s\n' "$BIT_FILE"
