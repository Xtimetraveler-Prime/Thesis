...
LOCAL_BUILD_DIR="$SCRIPT_DIR/build/m11_5_4_real_ip"
VIVADO_PROJECT_DIR="$LOCAL_BUILD_DIR/vivado_project"
VIVADO_TCL="$SCRIPT_DIR/vivado/create_m11_5_4_project.tcl"
LOG_FILE="$LOCAL_BUILD_DIR/m11_5_4_real_ip_vivado.log"
PASS_MARKER="M11.5.4 packed-M08 + real-HLS recurrent multi-tick passed:"

STAGE_ROOT="/tmp/neuromorphic_twin_rtl_${UID}/m11_5_4_real_ip"
...
RECURRENT_INTEGRATED_RTL="$STAGE_ROOT/recurrent_integrated_core_controller_v1.sv"
RECURRENT_BD_RTL="$STAGE_ROOT/recurrent_integrated_core_controller_bd_v1.v"
TB_FILE="$STAGE_ROOT/tb_neuromorphic_twin_m11_5_4.sv"
VECTOR_FILE="$STAGE_ROOT/generated_m11_5_4_integrated_vectors.svh"
VECTOR_EVIDENCE="$LOCAL_BUILD_DIR/generated_m11_5_4_integrated_vectors.svh"
...
if [[ -n "${HLS_PART:-}" && "$HLS_PART" != "$EXPECTED_PART" ]]; then
    echo "ERROR: M11.5.4 is frozen to $EXPECTED_PART, but HLS_PART=$HLS_PART" >&2
    exit 2
fi
...
SOURCE_FILES=(
    ...
    "$SCRIPT_DIR/tb/tb_neuromorphic_twin_m11_5_4.sv"
    "$VIVADO_TCL"
)
for path in "${SOURCE_FILES[@]}"; do
    if [[ ! -f "$path" ]]; then
        echo "ERROR: missing M11.5.4 source file: $path" >&2
        exit 3
    fi
done
...
cp "$SCRIPT_DIR/recurrent_integrated_core_controller_v1.sv" "$RECURRENT_INTEGRATED_RTL"
cp "$SCRIPT_DIR/recurrent_integrated_core_controller_bd_v1.v" "$RECURRENT_BD_RTL"
cp "$SCRIPT_DIR/tb/tb_neuromorphic_twin_m11_5_4.sv" "$TB_FILE"
...
printf 'M11.5.4 toolchain: Vivado %s\n' "$EXPECTED_VERSION"
printf 'M11.5.4 target part: %s\n' "$EXPECTED_PART"
printf 'M11.5.4 packaged HLS IP: %s\n' "$EXPECTED_VLNV"
printf 'M11.5.4 no-space staging directory: %s\n' "$STAGE_ROOT"

echo
echo '=== M11.5.4 recurrent packed M08 + real packaged HLS IP ==='
vivado -mode batch \
    -source "$VIVADO_TCL" \
    -tclargs \
    "$IP_REPO_DIR" \
...
if ! grep -Fq "$PASS_MARKER" "$LOG_FILE"; then
    echo "ERROR: Vivado returned without the M11.5.4 recurrent integrated pass marker." >&2
    exit 4
fi

PROJECT_FILE="$VIVADO_PROJECT_DIR/neuromorphic_twin_m11_5_4.xpr"
BD_FILE="$(find "$VIVADO_PROJECT_DIR" -type f -name 'neuromorphic_twin_m11_5_4.bd' -print -quit)"
if [[ ! -f "$PROJECT_FILE" || -z "$BD_FILE" ]]; then
    echo "ERROR: recurrent integrated simulation passed but expected Vivado artifacts were not found." >&2
    exit 4
fi

echo
echo 'M11.5.4 recurrent packed-M08 + real packaged HLS IP simulation completed successfully.'
