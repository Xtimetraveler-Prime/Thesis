...
LOCAL_BUILD_DIR="$SCRIPT_DIR/build/m12_3"
...
LOG_FILE="$LOCAL_BUILD_DIR/m12_3_vivado.log"
JOBS="${M12_3_JOBS:-4}"

STAGE_ROOT="/tmp/neuromorphic_twin_rtl_${UID}/m12_3"
...
if [[ -n "${HLS_PART:-}" && "$HLS_PART" != "$EXPECTED_PART" ]]; then
    echo "ERROR: M12.3 is frozen to $EXPECTED_PART, but HLS_PART=$HLS_PART" >&2
    exit 2
fi
if [[ ! "$JOBS" =~ ^[1-9][0-9]*$ ]]; then
    echo "ERROR: M12_3_JOBS must be a positive integer; got '$JOBS'." >&2
    exit 2
fi
...
SOURCE_FILES=(
    ...
    "$SCRIPT_DIR/m12_3_multitick_capture_controller_v1.sv"
    "$SCRIPT_DIR/m12_3_multitick_capture_controller_bd_v1.v"
    "$SCRIPT_DIR/vivado/create_m12_3_project.tcl"
    "$SCRIPT_DIR/check_m11_6_resources.py"
    "$PROJECT_DIR/examples/generate_m12_3_multitick_corpus.py"
)
for path in "${SOURCE_FILES[@]}"; do
    if [[ ! -f "$path" ]]; then
        echo "ERROR: missing M12.3 source file: $path" >&2
        exit 3
    fi
done
...
cp "$SCRIPT_DIR/m12_3_multitick_capture_controller_v1.sv" "$CAPTURE_RTL"
cp "$SCRIPT_DIR/m12_3_multitick_capture_controller_bd_v1.v" "$CAPTURE_BD_RTL"
cp "$SCRIPT_DIR/vivado/create_m12_3_project.tcl" "$VIVADO_TCL"

PYTHONPATH="$PROJECT_DIR/src${PYTHONPATH:+:$PYTHONPATH}" \
python3 "$PROJECT_DIR/examples/generate_m12_3_multitick_corpus.py" \
...
printf 'M12.3 toolchain: Vivado %s\n' "$EXPECTED_VERSION"
printf 'M12.3 target part: %s\n' "$EXPECTED_PART"
printf 'M12.3 packaged HLS IP: %s\n' "$EXPECTED_VLNV"
printf 'M12.3 implementation jobs: %s\n' "$JOBS"
printf 'M12.3 directed cases: 10\n'
printf 'M12.3 directed committed ticks: 40\n'
printf 'M12.3 no-space staging directory: %s\n' "$STAGE_ROOT"

echo
echo '=== M12.3 routed implementation + bitstream ==='
vivado -mode batch \
    -source "$VIVADO_TCL" \
    -tclargs \
    "$IP_REPO_DIR" \
...
for marker in \
    ...
    if ! grep -Fq "$marker" "$LOG_FILE"; then
        echo "ERROR: Vivado returned without expected M12.3 marker: $marker" >&2
        exit 4
    fi
done
...
XSA_FILE="$ARTIFACT_DIR/neuromorphic_twin_m12_3.xsa"
DCP_FILE="$ARTIFACT_DIR/neuromorphic_twin_m12_3_routed.dcp"
REQUIRED_REPORTS=(
...
for artifact in "$BIT_FILE" "$LTX_FILE" "$XSA_FILE" "$DCP_FILE"; do
    if [[ ! -s "$artifact" ]]; then
        echo "ERROR: required M12.3 artifact is missing or empty: $artifact" >&2
        exit 4
    fi
done
for report in "${REQUIRED_REPORTS[@]}"; do
    if [[ ! -s "$report" ]]; then
        echo "ERROR: required M12.3 report is missing or empty: $report" >&2
        exit 4
    fi
done
if [[ ! -s "$GOLDEN_DIR/manifest.json" ]]; then
    echo "ERROR: M12.3 golden corpus manifest is missing." >&2
    exit 4
fi
...
echo
echo 'M12.3 routed bitstream flow completed successfully.'
printf 'Bitstream: %s\n' "$BIT_FILE"
