# Program the M12.4 K26 bitstream once and capture all directed multi-tick cases.
if {$argc != 4} {
    error "usage: capture_m12_4_broad.tcl <bitstream> <ltx> <hardware_cases.tsv> <output_dir>"
}
...
foreach path [list $bitstream $probes $metadata] {
    if {![file exists $path]} {
        error "Required M12.4 hardware artifact does not exist: $path"
    }
}
...
if {[llength $meta_lines] < 2 || [lindex $meta_lines 0] ne "case_id\tcase_name\tsource_kind\tseed\tconfiguration_sha256\tneuron_count\taxon_count\tsynapse_count\troute_count\ttick_count"} {
    error "M12.4 hardware case metadata has an invalid header"
}
...
if {[llength $candidates] != 1} {
    error "M12.4 expected exactly one target hardware device; found: $candidates"
}
set dev [lindex $candidates 0]
current_hw_device $dev
set device_name [get_property NAME $dev]
puts "M12.4 hardware device: $device_name"

set_property PROGRAM.FILE $bitstream $dev
set_property PROBES.FILE $probes $dev
program_hw_devices $dev
refresh_hw_device $dev
puts "M12.4 bitstream programmed successfully."

set vios [get_hw_vios -quiet]
if {[llength $vios] != 1} {
    error "M12.4 expected exactly one VIO core after programming; found: $vios"
}
...
if {[probe_uint $p_reset_released] != 0} {
    error "M12.4 synchronized reset was not asserted before heartbeat test"
}
...
if {$heartbeat_before == $heartbeat_after} {
    error "M12.4 PL clock heartbeat did not advance: value=$heartbeat_before"
}
puts "M12.4 PL clock heartbeat advanced: $heartbeat_before -> $heartbeat_after"

set_probe_uint $p_resetn 1
after 20
refresh_hw_vio $vio
if {[probe_uint $p_reset_released] != 1} {
    error "M12.4 proc_sys_reset did not release the capture domain"
}
puts "M12.4 local capture reset released through VIO."
...
foreach record $case_lines {
    ...
    if {[llength $fields] != 10} {
        error "Invalid M12.4 hardware metadata row: $record"
    }
    ...
    if {
        ...
        error "Invalid integer M12.4 metadata row: $record"
    }
    ...
    pulse_probe $p_start
    wait_for_input $vio $p_step_ready 1 1000 2 "M12.4 case preload/reset"
    refresh_hw_vio $vio
    set phase [probe_uint $p_phase]
    set selected_case $phase
    if {$selected_case != $case_id} {
        error "M12.4 case-select witness mismatch: requested=$case_id observed=$selected_case phase=$phase"
    }
    if {[probe_uint $p_tick] != 0 || [probe_uint $p_capture_fault] != 0} {
        error "M12.4 case $case_id did not reach clean tick zero: tick=[probe_uint $p_tick] capture_fault=[probe_uint $p_capture_fault] code=[probe_uint $p_capture_fault_code] phase=$phase"
    }
    ...
    for {set expected_tick 1} {$expected_tick <= $tick_count} {incr expected_tick} {
        ...
        for {set attempt 0} {$attempt < 1000} {incr attempt} {
            ...
            if {[probe_uint $p_capture_fault] != 0} {
                close $fh
                error "M12.4 capture shell fault for case $case_id tick=$expected_tick: code=[probe_uint $p_capture_fault_code] phase=[probe_uint $p_phase] core_fault=[probe_uint $p_core_fault_code]"
            }
        ...
        if {!$reached} {
            close $fh
            error "Timed out waiting for M12.4 post-commit trace window: case=$case_id expected_tick=$expected_tick observed_tick=[probe_uint $p_tick] phase=[probe_uint $p_phase]"
        }
        ...
        if {[probe_uint $p_done] != $expected_done} {
            close $fh
            error "M12.4 capture_done mismatch: case=$case_id tick=$expected_tick expected=$expected_done actual=[probe_uint $p_done]"
        }
        ...
        incr captured_ticks
        puts "M12.4 captured physical case $case_id tick $expected_tick/$tick_count: external=$external_count consumed=$consumed_count routed=$routed_count bank=$current_bank"
    }
    ...
    close $fh
    incr captured_cases
    puts "M12.4 captured physical case $case_id complete: name=$case_name ticks=$tick_count output=$output_json"
}

if {$captured_cases != [llength $case_lines]} {
    error "M12.4 captured case count mismatch: captured=$captured_cases metadata=[llength $case_lines]"
}
puts "M12.4 physical broad deterministic suite capture completed successfully: cases=$captured_cases ticks=$captured_ticks output=$output_dir"
close_hw_manager
