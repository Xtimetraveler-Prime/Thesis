# Recreate and simulate the M11.5.4 recurrent packed-M08 + real HLS integration.
if {$argc != 13} {
    error "usage: create_m11_5_4_project.tcl <ip_repo_dir> <project_dir> <target_part> <expected_vlnv> <decoder_rtl> <phase_b_rtl> <neuron_rtl> <integrated_rtl> <route_rtl> <recurrent_integrated_rtl> <recurrent_bd_rtl> <tb_file> <vector_include>"
}
...
set vector_include           [file normalize [lindex $argv 12]]

set project_name "neuromorphic_twin_m11_5_4"
set bd_name "neuromorphic_twin_m11_5_4"
set controller_name "recurrent_integrated_core_controller_bd_v1"

foreach path [list $ip_repo_dir $decoder_rtl $phase_b_rtl $neuron_rtl $integrated_rtl $route_rtl $recurrent_integrated_rtl $recurrent_bd_rtl $tb_file $vector_include] {
    if {![file exists $path]} {
        error "Required M11.5.4 input does not exist: $path"
    }
}
...
proc connect_verified_pair {controller_pin hls_pin} {
    ...
    if {[llength $cp] != 1 || [llength $hp] != 1} {
        error "Missing M11.5.4 integration pin: controller=$controller_pin ($cp), hls=$hls_pin ($hp)"
    }
    ...
    if {[llength $cn] != 1 || [llength $hn] != 1 || [lindex $cn 0] ne [lindex $hn 0]} {
        error "M11.5.4 integration pin connectivity mismatch: controller=$controller_pin nets=$cn, hls=$hls_pin nets=$hn"
    }
    puts "M11.5.4 connected controller_0/$controller_pin -> neuron_step_v1_0/$hls_pin on [lindex $hn 0]"
}
...
set external_controller_pins {
    ...
    last_consumed_recurrent_count last_routed_count
    ...
    debug_re debug_addr debug_rvalid debug_config_rdata debug_state_rdata debug_accum_rdata debug_spike_rdata
...
save_bd_design
puts ""
puts "M11.5.4 recurrent packed-M08 real-HLS block design validated successfully."
puts "Controller module reference: $controller_name"
puts "Packaged HLS IP: $expected_vlnv"
puts "Recurrent integration: current-bank copy -> Phase B -> real HLS -> spike scan -> route bank swap"
...
set_property top tb_neuromorphic_twin_m11_5_4 [get_filesets sim_1]
update_compile_order -fileset sim_1

puts ""
puts "=== M11.5.4 recurrent packed M08 + real HLS behavioral simulation ==="
launch_simulation -simset sim_1 -mode behavioral
run all
close_sim

puts ""
puts "M11.5.4 recurrent packed-M08 + real packaged-HLS Vivado simulation flow completed."
