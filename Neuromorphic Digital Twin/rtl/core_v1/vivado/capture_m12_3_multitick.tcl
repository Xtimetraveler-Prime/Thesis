...
if {$argc != 4} {
    error "usage: capture_m12_3_multitick.tcl <bitstream> <ltx> <hardware_cases.tsv> <output_dir>"
}
...
foreach path [list $bitstream $probes $metadata] {
    if {![file exists $path]} {
        error "Required M12.3 hardware artifact does not exist: $path"
    }
}
...
if {[llength $meta_lines] < 2 || [lindex $meta_lines 0] ne "case_id\tcase_name\tneuron_count\ttick_count"} {
    error "M12.3 hardware case metadata has an invalid header"
}
...
if {[llength $candidates] != 1} {
    error "M12.3 expected exactly one target hardware device; found: $candidates"
}
set dev [lindex $candidates 0]
current_hw_device $dev
set device_name [get_property NAME $dev]
puts "M12.3 hardware device: $device_name"

set_property PROGRAM.FILE $bitstream $dev
set_property PROBES.FILE $probes $dev
program_hw_devices $dev
refresh_hw_device $dev
puts "M12.3 bitstream programmed successfully."

set vios [get_hw_vios -quiet]
if {[llength $vios] != 1} {
    error "M12.3 expected exactly one VIO core after programming; found: $vios"
}
...
if {[probe_uint $p_reset_released] != 0} {
    error "M12.3 synchronized reset was not asserted before heartbeat test"
}
...
if {$heartbeat_before == $heartbeat_after} {
    error "M12.3 PL clock heartbeat did not advance: value=$heartbeat_before"
}
puts "M12.3 PL clock heartbeat advanced: $heartbeat_before -> $heartbeat_after"

set_probe_uint $p_resetn 1
after 20
refresh_hw_vio $vio
if {[probe_uint $p_reset_released] != 1} {
    error "M12.3 proc_sys_reset did not release the capture domain"
}
puts "M12.3 local capture reset released through VIO."
