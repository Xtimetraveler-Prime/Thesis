# Program the M12.1.3 K26 bitstream and capture four committed physical ticks.
if {$argc != 3} {
    error "usage: capture_m12_1_3_trace.tcl <bitstream> <ltx> <output_json>"
}
set bitstream [file normalize [lindex $argv 0]]
set probes    [file normalize [lindex $argv 1]]
set output_arg [lindex $argv 2]
file mkdir [file dirname $output_arg]
set output_json [file normalize $output_arg]
foreach path [list $bitstream $probes] {
    if {![file exists $path]} {
        error "Required M12.1.3 hardware artifact does not exist: $path"
    }
}

set SCENARIO_ID "m11_5_4_recurrent_chain_physical_trace_v1"
set NEURON_COUNT 3
set TICK_COUNT 4
...
if {[llength $candidates] != 1} {
    error "M12.1.3 expected exactly one target hardware device; found: $candidates"
}
set dev [lindex $candidates 0]
current_hw_device $dev
set device_name [get_property NAME $dev]
puts "M12.1.3 hardware device: $device_name"

set_property PROGRAM.FILE $bitstream $dev
set_property PROBES.FILE $probes $dev
program_hw_devices $dev
refresh_hw_device $dev
puts "M12.1.3 bitstream programmed successfully."

set vios [get_hw_vios -quiet]
if {[llength $vios] != 1} {
    error "M12.1.3 expected exactly one VIO core after programming; found: $vios"
}
...
# Slow-host read transaction. trace_response_seq is sticky and increments for
# every accepted bridge response, preventing JTAG polling from missing a 10 ns
# response pulse.
...
# Initialize all command probes low, assert reset, and prove the physical PL
# clock is running before releasing the capture/HLS domains.
...
if {[probe_uint $p_reset_released] != 0} {
    error "M12.1.3 synchronized reset was not asserted before heartbeat test"
}
...
if {$heartbeat_before == $heartbeat_after} {
    error "M12.1.3 PL clock heartbeat did not advance: value=$heartbeat_before"
}
puts "M12.1.3 PL clock heartbeat advanced: $heartbeat_before -> $heartbeat_after"

set_probe_uint $p_resetn 1
after 20
refresh_hw_vio $vio
if {[probe_uint $p_reset_released] != 1} {
    error "M12.1.3 proc_sys_reset did not release the capture domain"
}
puts "M12.1.3 local capture reset released through VIO."

pulse_probe $p_start
wait_for_input $vio $p_start_seen 1 100 2 "capture_start witness"
wait_for_input $vio $p_step_ready 1 500 2 "post-preload step-ready state"
refresh_hw_vio $vio
if {[probe_uint $p_tick] != 0 || [probe_uint $p_capture_fault] != 0} {
    error "M12.1.3 preload/reset did not reach a clean tick-zero boundary: tick=[probe_uint $p_tick] capture_fault=[probe_uint $p_capture_fault] code=[probe_uint $p_capture_fault_code] phase=[probe_uint $p_phase]"
}
puts "M12.1.3 preload/reset complete; host-stepped physical execution is ready."

set fh [open $output_json w]
puts $fh "{"
puts $fh "  \"schema\": \"neuromorphic-twin-physical-fpga-trace-v1\","
puts $fh "  \"scenario_id\": [json_quote $SCENARIO_ID],"
...
for {set expected_tick 1} {$expected_tick <= $TICK_COUNT} {incr expected_tick} {
    ...
    # Wait specifically for the immutable post-commit trace window for this tick.
    set reached 0
    for {set attempt 0} {$attempt < 500} {incr attempt} {
        refresh_hw_vio $vio
        if {[probe_uint $p_capture_fault] != 0} {
            close $fh
            error "M12.1.3 capture shell fault while executing tick $expected_tick: code=[probe_uint $p_capture_fault_code] phase=[probe_uint $p_phase] core_fault=[probe_uint $p_core_fault_code]"
        }
    ...
    if {!$reached} {
        close $fh
        error "Timed out waiting for post-commit trace window at tick $expected_tick: tick=[probe_uint $p_tick] phase=[probe_uint $p_phase]"
    }
    ...
    # Phase F leaves newly routed events in current_bank. The consumed input
    # prefix remains readable in the opposite bank and is bounded by the sticky
    # last_consumed_recurrent_count metadata.
    if {$current_bank == 0} {
        set routed_space 5
...
refresh_hw_vio $vio
if {[probe_uint $p_done] != 1 || [probe_uint $p_capture_fault] != 0} {
    error "M12.1.3 final capture status invalid: done=[probe_uint $p_done] fault=[probe_uint $p_capture_fault] code=[probe_uint $p_capture_fault_code]"
}
puts "M12.1.3 physical trace capture completed successfully: $output_json"
close_hw_manager
