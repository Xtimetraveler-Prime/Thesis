# Recreate and simulate the M11.5.5 trace-capable recurrent core with real HLS IP.
if {$argc != 14} {
    error "usage: create_m11_5_5_trace_project.tcl <ip_repo_dir> <project_dir> <target_part> <expected_vlnv> <decoder_rtl> <phase_b_rtl> <neuron_rtl> <integrated_rtl> <route_rtl> <recurrent_integrated_rtl> <recurrent_bd_rtl> <tb_file> <m54_include> <m55_include>"
}
...
set tb_file                  [file normalize [lindex $argv 11]]
set m54_include              [file normalize [lindex $argv 12]]
set m55_include              [file normalize [lindex $argv 13]]

set project_name "neuromorphic_twin_m11_5_5_trace"
set bd_name "neuromorphic_twin_m11_5_5"
set controller_name "recurrent_integrated_core_controller_bd_v1"

foreach path [list $ip_repo_dir $decoder_rtl $phase_b_rtl $neuron_rtl $integrated_rtl $route_rtl $recurrent_integrated_rtl $recurrent_bd_rtl $tb_file $m54_include $m55_include] {
    if {![file exists $path]} {
        error "Required M11.5.5 trace input does not exist: $path"
    }
}
...
proc connect_verified_pair {controller_pin hls_pin} {
    ...
    if {[llength $cp] != 1 || [llength $hp] != 1} {
        error "Missing M11.5.5 trace integration pin: controller=$controller_pin ($cp), hls=$hls_pin ($hp)"
    }
    ...
    if {[llength $cn] != 1 || [llength $hn] != 1 || [lindex $cn 0] ne [lindex $hn 0]} {
        error "M11.5.5 trace connectivity mismatch: controller=$controller_pin nets=$cn, hls=$hls_pin nets=$hn"
    }
}
...
create_bd_cell -type module -reference $controller_name controller_0
create_bd_cell -type ip -vlnv $expected_vlnv neuron_step_v1_0
...
validate_bd_design
save_bd_design
puts "M11.5.5 trace real-HLS block design validated successfully."
...
add_files -fileset sim_1 -norecurse [list $tb_file $m54_include $m55_include]
set_property file_type SystemVerilog [get_files $tb_file]
set_property file_type "Verilog Header" [get_files $m54_include]
set_property file_type "Verilog Header" [get_files $m55_include]
set_property include_dirs [list [file dirname $m54_include]] [get_filesets sim_1]
set_property top tb_neuromorphic_twin_m11_5_5_trace [get_filesets sim_1]
update_compile_order -fileset sim_1

puts ""
puts "=== M11.5.5 trace snapshot + real HLS behavioral simulation ==="
launch_simulation -simset sim_1 -mode behavioral
run all
close_sim

puts "M11.5.5 trace real-HLS Vivado simulation flow completed."
