...
// M12.5 characterization multi-tick/recurrent physical capture controller.
//
// Python independently generates the complete per-case FPGA load image,
// per-tick external schedule, and host-side golden timeline.  This controller can see ONLY the load-image arrays in
// generated_m12_4_broad_cases.svh; expected outputs remain host-side.
//
...
// does not change.
//
// Unlike the fixed reset-state M12.1 workload, M12.4 must test arbitrary
...
module m12_5_characterization_capture_controller_v1 (
    ...
    `include "generated_m12_4_broad_cases.svh"
    ...
    assign capture_busy =
        ...
        (state != S_FAIL);
    assign step_ready = (state == S_READY_TICK) || ((state == S_CAPTURE_HOLD) && !capture_done);
    ...
    // Load/control mux. Every M12_4_* array here is an INPUT/load-image array;
    ...
    always_comb begin
        ...
        case (state)
            S_LOAD_CONFIG: begin
                config_we = 1'b1;
                config_wdata = M12_4_CONFIG_WORDS[
                    case_config_offset + load_index
                ];
            end
            S_LOAD_FORMAT: begin
                format_we = 1'b1;
                format_wdata = M12_4_FORMAT_WORDS[
                    case_format_offset + load_index
                ];
            end
            S_LOAD_SYNAPSE: begin
                synapse_we = 1'b1;
                synapse_wdata = M12_4_SYNAPSE_WORDS[
                    case_synapse_offset + load_index
                ];
            end
            S_LOAD_WEIGHT_ROW: begin
                row_we = 1'b1;
                row_wdata = M12_4_WEIGHT_ROWS[
                    case_weight_row_offset + load_index
                ];
            end
            S_LOAD_ROUTE_ROW: begin
                route_row_we = 1'b1;
                route_row_wdata = M12_4_ROUTE_ROWS[
                    case_route_row_offset + load_index
                ];
            end
            S_LOAD_ROUTE_TARGET: begin
                route_target_we = 1'b1;
                route_target_wdata = M12_4_ROUTE_TARGETS[
                    case_route_target_offset + load_index
                ];
            end
            S_RESET_PULSE: core_reset_start = 1'b1;
            S_LOAD_STATE: begin
                state_we = 1'b1;
                state_wdata = M12_4_INITIAL_STATE_WORDS[
                    case_state_offset + load_index
                ];
            end
            S_LOAD_EXTERNAL: begin
                external_we = 1'b1;
                external_wdata = M12_4_EXTERNAL_EVENTS[
    ...
    // Passive M12.5 measurement only. These registers never feed the
            ...
            case (state)
                S_IDLE: begin
                    if (capture_start_pulse) begin
                        ...
                        tick_index <= 8'd0;
                        watchdog <= 24'd0;
                        if (trace_read_addr >= M12_4_CASE_COUNT) begin
                            capture_fault <= 1'b1;
                ...
                S_RESET_WAIT: begin
                    if (core_fault) begin
                        ...
                        if ((core_tick != 32'd0) || (recurrent_current_count != 13'd0)) begin
                            ...
                            // M12.4 initial state is intentionally loaded AFTER
                ...
                S_TICK_WAIT: begin
                    if (core_fault) begin
                        ...
                        watchdog <= 24'd0;
                        if (core_tick != ({24'd0, tick_index} + 32'd1)) begin
                            capture_fault <= 1'b1;
